    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        focheck.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, final boolean itemcheck) {
                database.child("Data").child(data.getId()).addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(DataSnapshot dataSnapshot) {

                        if (focheck.isChecked()) {

                            database.child("Data").child(data.getId()).child("checkb").setValue(true);
                            todoChekd.setCheckb(true);
                            // focheck.setChecked(true);
                            // listAdapter.notifyDataSetChanged();
                        } else {

                            database.child("Data").child(data.getId()).child("checkb").setValue(false);
